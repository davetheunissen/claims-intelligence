...
const useStyles = makeStyles({
  ...
  agentCard: {
    ...
    backgroundImage: "linear-gradient(135deg, rgba(43,197,180,0.10), rgba(35,105,186,0.06))",
    minHeight: "200px",
  },
  cursor: {
  ...
  markdownRoot: {
    ...
    "& h1, & h2, & h3, & h4": { fontSize: "14px", fontWeight: 600, margin: "6px 0 2px" },
  },
});
...
function policyStatusColor(status: string): "success" | "danger" | "warning" | "informative" {
...
function MemberPolicyCard({ policy }: { policy: MemberPolicySnapshot | undefined }) {
  ...
  const deductible =
    typeof policy.applicable_deductible === "number"
  ...
  return (
    <Card style={{ padding: "14px" }}>
      ...
      >
        ...
        <Badge appearance="filled" color={policyStatusColor(policy.status)}>
        ...
        <div style={{ marginTop: "10px" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
            {policy.applicable_endorsements.map((e) => (
              <Badge key={e} appearance="outline" color="informative">
                {e}
              </Badge>
            ))}
          </div>
      ...
      {policy.policy_excerpts && policy.policy_excerpts.length > 0 && (
        <div style={{ marginTop: "12px", display: "flex", flexDirection: "column", rowGap: "8px" }}>
          {policy.policy_excerpts.map((px: PolicyExcerpt) => (
            <div
              key={px.id}
              style={{
                padding: "8px 10px",
            ...
            >
              ...
              <Body1 style={{ color: tokens.colorNeutralForeground2, fontSize: "13px" }}>
                {px.snippet}
              </Body1>
            </div>
          ))}
...
export function Step6Recommendation({ mode, onNext, onEdit }: Props) {
  ...
  async function handoffToSIU() {
    ...
    try {
      ...
      const res = await claimsdemo.siuHandoff(claimId, snapshot, decisionNote.trim() || undefined);
      setDisposition(res.disposition);
      setDecisionNote("");
      void refreshAudit();
  ...
  const doneSummary = data
    ? disposition
      ? `${decisionLabels[disposition.decision]} · ${data.recommendation.verdict} (${Math.round(data.recommendation.confidence * 100)}% confidence)`
      : `${data.recommendation.verdict} (${Math.round(data.recommendation.confidence * 100)}% confidence)`
    : undefined;
  ...
  return (
    <JourneySection
      ...
      techStack={
        fromFixture
          ? "Local fixture data"
          : "Foundry Agent Service · Azure AI Search · GPT-5.1"
      }
      techDetails={
        fromFixture
        ...
        <>
          ...
          <div className={styles.grid}>
            ...
            >
              ...
              <div className={styles.markdownRoot} style={{ marginTop: "12px", lineHeight: 1.6 }}>
          ...
          <div className={styles.decisionCard}>
            ...
            {!dispositionLoaded && (
              <Caption1 className={styles.decisionMeta}>Loading saved decision…</Caption1>
            )}
            ...
            {dispositionLoaded && !disposition && (
              <>
                <Caption1 className={styles.decisionMeta}>
                  Record the adjuster outcome before drafting the customer letter. The recommendation
                  will be saved with the decision.
                </Caption1>
                <Textarea
                  value={decisionNote}
                ...
                />
                <Caption1 className={styles.decisionMeta}>
                  Pick a decision, then click <strong>Save decision &amp; draft letter</strong>{" "}
                  below to record it and continue.
                </Caption1>
                <div className={styles.decisionRow}>
                  {(
                    [
          ...
          <div className={styles.activityCard}>
            ...
            {auditEvents.length === 0 ? (
              <Caption1 className={styles.decisionMeta}>No activity recorded yet.</Caption1>
            ) : (
